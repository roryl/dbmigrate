<cfcomponent output="false" mixin="none" environment="design,development,maintenance">
	...
	<cffunction name="migrateTo" access="public" returntype="string" hint="migrates database to a specified version">
		...
		<cfelse>
			<cfset loc.sqlPath = "/#request.dbMigrationPath#/sql">
	...
	<cffunction name="getAvailableMigrations" access="public" returntype="array" hint="searches migrate folder for migrations">
		<cfset var loc = {}>
		...
		<cfset loc.migrationsPath = "/#request.dbMigrationPath#/migrate/">
		...
		<cfloop query="qMigrationFiles">
			<cfif REFind(loc.migrationRE,Name)>
				...
				<cftry>
					...
					<cfset loc.migration.cfc = createObject("#request.dbMigrationMapping#.migrate.#loc.migration.cfcFile#").init(variables.datasource)> 
					<!--- <cfset loc.migration.cfc = $createObjectFromRoot(path="dbmigrate.dbmigrate.DBMigrate.db.migrate",fileName=loc.migration.cfcfile, method="init")> --->
					...
					<cfcatch type="any"><cfdump var="#cfcatch#" abort="true" /><cfset loc.migration.loadError = CFCATCH.Message></cfcatch>
				</cftry>
	...
	<cffunction name="$createObjectFromRoot" returntype="any" access="private" output="false">
		...
		<cfreturn createObject("component","#arguments.path#.#arguments.fileName#").init(variables.datasource)>
		<!--- <cfscript>
			var loc = {};
	...
	<cffunction name="$copyTemplateMigrationAndRename" displayname="$copyTemplateMigrationAndRename" access="private" returntype="string">
		...
		<cfset loc.templateFile = GetDirectoryFromPath(getCurrentTemplatePath()) & "templates/" & arguments.templateName & ".cfc">
		...
		<cftry>
			...
			<cfset loc.templateContent = replace(loc.templateContent, "[extends]", "dbmigrate.dbmigrate.Migration")>
	...
	<cffunction name="$getVersionsPreviouslyMigrated" access="private" returntype="string">
		<cfset var loc = {}>
		<cftry>
			<cfquery name="loc.qMigratedVersions" datasource="#variables.datasource#">
